...
async function translateBerndeutschToHochdeutsch(text) {
    try {
        const response = await fetch('/api/openai-assistant', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                message: `Übersetze diesen Berndeutsch/Schweizerdeutsch Text ins Hochdeutsche, behalte dabei den Sinn und die Emotion bei:\n\n"${text}"`,
                context: {
                    requestType: 'translation',
                    sourceLanguage: 'Berndeutsch/Schweizerdeutsch',
                    targetLanguage: 'Hochdeutsch'
                }
            })
        });
...
// Berndeutsch Speech-to-Text mit Auto-Übersetzung
let berndeutschRecognition = null;

function initBerndeutschSpeechRecognition() {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
        berndeutschRecognition = new (window.SpeechRecognition || window.webkitSpeechRecognition)();
        berndeutschRecognition.continuous = true;
        berndeutschRecognition.interimResults = true;
        berndeutschRecognition.lang = 'de-CH'; // Schweizerdeutsch
        
        console.log('🎤 Berndeutsch Speech Recognition initialisiert');
        return true;
    }
    return false;
}

// Erweiterte Voice Summary mit Berndeutsch-Übersetzung
function addBerndeutschVoiceSummaryInterface() {
    const coachKIResponse = DOM.find('#coachKIResponse');
    if (!coachKIResponse || DOM.find('#berndeutschVoiceSection')) return;

    const berndeutschHTML = `
        <div id="berndeutschVoiceSection" style="margin-top: 20px; padding: 20px; background: rgba(245, 158, 11, 0.1); border-radius: 12px; border: 2px solid rgba(245, 158, 11, 0.3);">
            <h4 style="color: #92400e; margin-bottom: 15px; display: flex; align-items: center;">
                🏔️ Berndeutsch → Hochdeutsch Zusammenfassung
            </h4>
            
            <div style="display: flex; gap: 10px; margin-bottom: 15px; align-items: center; flex-wrap: wrap;">
                <button id="startBerndeutschBtn" onclick="startBerndeutschRecording()" style="background: #f59e0b; color: white; border: none; padding: 12px 20px; border-radius: 8px; font-weight: 600; cursor: pointer;">
                    🎤 Berndeutsch aufnehmen
                </button>
                <button id="stopBerndeutschBtn" onclick="stopBerndeutschRecording()" style="background: #ef4444; color: white; border: none; padding: 12px 20px; border-radius: 8px; font-weight: 600; cursor: pointer; display: none;">
                    ⏹️ Stoppen
                </button>
                <button id="translateBtn" onclick="translateToHochdeutsch()" style="background: #10b981; color: white; border: none; padding: 12px 20px; border-radius: 8px; font-weight: 600; cursor: pointer; display: none;">
                    🔄 Ins Hochdeutsche übersetzen
                </button>
                <span id="berndeutschStatus" style="color: #92400e; font-weight: 600;"></span>
            </div>
            
            <div id="berndeutschContainer" style="display: none; margin-bottom: 15px;">
                <div style="background: white; border: 2px solid rgba(245, 158, 11, 0.3); border-radius: 8px; padding: 15px; margin-bottom: 10px;">
                    <div style="color: #92400e; font-weight: 600; margin-bottom: 10px;">🏔️ Berndeutsch Original:</div>
                    <div id="berndeutschText" style="line-height: 1.6; color: #374151; font-style: italic;"></div>
                </div>
                
                <div id="hochdeutschContainer" style="background: white; border: 2px solid rgba(16, 185, 129, 0.3); border-radius: 8px; padding: 15px; display: none;">
                    <div style="color: #065f46; font-weight: 600; margin-bottom: 10px;">🇩🇪 Hochdeutsch Übersetzung:</div>
                    <div id="hochdeutschText" style="line-height: 1.6; color: #374151;"></div>
                </div>
            </div>
            
            <div id="berndeutschActions" style="display: none; gap: 10px;">
                <button onclick="editHochdeutschTranslation()" style="background: #6b7280; color: white; border: none; padding: 10px 15px; border-radius: 6px; cursor: pointer;">
                    ✏️ Übersetzung bearbeiten
                </button>
                <button onclick="sendHochdeutschToCoachee()" style="background: #10b981; color: white; border: none; padding: 10px 15px; border-radius: 6px; cursor: pointer; font-weight: 600;">
                    📤 Hochdeutsch an Coachee senden
                </button>
                <button onclick="clearBerndeutschSummary()" style="background: #ef4444; color: white; border: none; padding: 10px 15px; border-radius: 6px; cursor: pointer;">
                    🗑️ Löschen
                </button>
            </div>
        </div>
    ...
    coachKIResponse.insertAdjacentHTML('beforeend', berndeutschHTML);
}

// Berndeutsch Aufnahme starten
window.startBerndeutschRecording = function() {
    if (!berndeutschRecognition) {
        if (!initBerndeutschSpeechRecognition()) {
            Utils.showToast('Speech Recognition nicht verfügbar für Berndeutsch.', 'error');
            return;
        }
    }

    // UI Updates
    DOM.find('#startBerndeutschBtn').style.display = 'none';
    DOM.find('#stopBerndeutschBtn').style.display = 'inline-block';
    DOM.find('#berndeutschStatus').textContent = 'Berndeutsch Aufnahme läuft... 🔴';
    DOM.find('#berndeutschContainer').style.display = 'block';
    DOM.find('#berndeutschText').textContent = 'Bitte auf Berndeutsch sprechen...';

    // Timer
    const startTime = Date.now();
    const timer = setInterval(() => {
        const elapsed = Math.floor((Date.now() - startTime) / 1000);
        DOM.find('#berndeutschStatus').textContent = `Berndeutsch Aufnahme... 🔴 ${elapsed}s`;
    }, 1000);

    // Speech Recognition Events
    berndeutschRecognition.onresult = function(event) {
        let finalTranscript = '';
        let interimTranscript = '';
        ...
        const berndeutschEl = DOM.find('#berndeutschText');
        if (berndeutschEl) {
            const currentFinal = berndeutschEl.getAttribute('data-final') || '';
            berndeutschEl.setAttribute('data-final', currentFinal + finalTranscript);
            berndeutschEl.innerHTML = 
                `<span style="color: #374151;">${currentFinal + finalTranscript}</span>` +
                `<span style="color: #9ca3af; font-style: italic;">${interimTranscript}</span>`;
        }
    };

    berndeutschRecognition.onerror = function(event) {
        console.error('Berndeutsch Speech Recognition Error:', event.error);
        Utils.showToast(`Berndeutsch Spracherkennung Fehler: ${event.error}`, 'error');
        clearInterval(timer);
    };

    berndeutschRecognition.start();
    Utils.showToast('Berndeutsch Aufnahme gestartet', 'success');
};

// Berndeutsch Aufnahme stoppen
window.stopBerndeutschRecording = function() {
    if (berndeutschRecognition) {
        berndeutschRecognition.stop();
    }

    // UI Updates
    DOM.find('#startBerndeutschBtn').style.display = 'inline-block';
    DOM.find('#stopBerndeutschBtn').style.display = 'none';
    DOM.find('#translateBtn').style.display = 'inline-block';
    DOM.find('#berndeutschStatus').textContent = 'Berndeutsch Aufnahme beendet ✅';

    // Finale Berndeutsch Transkription
    const berndeutschEl = DOM.find('#berndeutschText');
    if (berndeutschEl) {
        const finalText = berndeutschEl.getAttribute('data-final') || berndeutschEl.textContent;
        berndeutschEl.innerHTML = `<span style="color: #374151;">${finalText.trim()}</span>`;
    }

    Utils.showToast('Berndeutsch Aufnahme beendet. Klicke "Übersetzen" für Hochdeutsch.', 'success');
};

// Ins Hochdeutsche übersetzen
window.translateToHochdeutsch = function() {
    const berndeutschEl = DOM.find('#berndeutschText');
    if (!berndeutschEl) return;

    const berndeutschText = berndeutschEl.textContent.trim();
    if (!berndeutschText || berndeutschText === 'Bitte auf Berndeutsch sprechen...') {
        Utils.showToast('Kein Berndeutsch Text vorhanden.', 'error');
        return;
    }

    // Button-Status
    const translateBtn = DOM.find('#translateBtn');
    if (translateBtn) {
    ...
    translateBerndeutschToHochdeutsch(berndeutschText)
        .then(hochdeutschText => {
            if (hochdeutschText) {
                // Hochdeutsch anzeigen
                DOM.find('#hochdeutschContainer').style.display = 'block';
                DOM.find('#hochdeutschText').textContent = hochdeutschText;
                DOM.find('#berndeutschActions').style.display = 'flex';
                
                Utils.showToast('Übersetzung ins Hochdeutsche erfolgreich!', 'success');
            } else {
                Utils.showToast('Übersetzung fehlgeschlagen. Versuche es erneut.', 'error');
            }
        })
        .catch(error => {
            console.error('Translation Error:', error);
            Utils.showToast('Übersetzungsfehler. Prüfe die OpenAI Verbindung.', 'error');
        })
        .finally(() => {
            // Button zurücksetzen
            if (translateBtn) {
                translateBtn.textContent = '🔄 Ins Hochdeutsche übersetzen';
...
window.editHochdeutschTranslation = function() {
    const hochdeutschEl = DOM.find('#hochdeutschText');
    if (!hochdeutschEl) return;

    const currentText = hochdeutschEl.textContent || '';
    const textarea = DOM.create('textarea', {
        style: 'width: 100%; min-height: 100px; padding: 10px; border: 2px solid #10b981; border-radius: 6px; font-family: inherit;',
        value: currentText
    });

    hochdeutschEl.parentNode.replaceChild(textarea, hochdeutschEl);
    textarea.focus();

    // Save/Cancel Buttons
    const actions = DOM.find('#berndeutschActions');
    actions.innerHTML = `
        <button onclick="saveHochdeutschEdit()" style="background: #10b981; color: white; border: none; padding: 10px 15px; border-radius: 6px; cursor: pointer; font-weight: 600;">
            💾 Speichern
        </button>
        <button onclick="cancelHochdeutschEdit()" style="background: #6b7280; color: white; border: none; padding: 10px 15px; border-radius: 6px; cursor: pointer;">
            ❌ Abbrechen
        </button>
    `;
};

// Hochdeutsch Bearbeitung speichern
window.saveHochdeutschEdit = function() {
    const textarea = DOM.find('#hochdeutschContainer textarea');
    if (!textarea) return;
    ...
    Utils.showToast('Hochdeutsche Übersetzung gespeichert', 'success');
};

// Hochdeutsch an Coachee senden
window.sendHochdeutschToCoachee = function() {
    const hochdeutschEl = DOM.find('#hochdeutschText');
    if (!hochdeutschEl) return;

    const hochdeutschText = hochdeutschEl.textContent.trim();
    if (!hochdeutschText) {
        Utils.showToast('Keine Hochdeutsche Übersetzung vorhanden.', 'error');
        return;
    }
    ...
    const formattedSummary = `📋 Coach-Zusammenfassung (aus Berndeutsch übersetzt):\n\n${hochdeutschText}`;
    ...
    Utils.showToast('Hochdeutsche Zusammenfassung an Coachee gesendet!', 'success');
    console.log('📤 Berndeutsch → Hochdeutsch Zusammenfassung gesendet');

    // Cleanup
    clearBerndeutschSummary();
};

// Berndeutsch Summary löschen
window.clearBerndeutschSummary = function() {
    const berndeutschSection = DOM.find('#berndeutschVoiceSection');
    if (berndeutschSection) {
        berndeutschSection.remove();
    }
};

// Coach-KI Response Display erweitern für Berndeutsch
const originalCoachKIDisplayResponse = CoachKI.displayResponse;
CoachKI.displayResponse = function(response) {
    // Original Response anzeigen
    originalCoachKIDisplayResponse.call(this, response);
    
    // Berndeutsch Voice Summary Interface hinzufügen
    setTimeout(() => {
        addVoiceSummaryInterface(); // Original
        addBerndeutschVoiceSummaryInterface(); // Neu
    }, 100);
};

// Quick-Access Button für Berndeutsch
function addBerndeutschQuickButton() {
    const quickAccess = DOM.find('.quick-access');
    if (quickAccess && !DOM.find('#berndeutschQuickBtn')) {
        const berndeutschBtn = DOM.create('button', {
            id: 'berndeutschQuickBtn',
            className: 'quick-btn',
            innerHTML: '🏔️ Berndeutsch → Hochdeutsch',
            style: 'background: #f59e0b; color: white;'
        });
        
        DOM.on(berndeutschBtn, 'click', () => {
            CoachInterface.showTab('coachki');
            setTimeout(() => {
                const input = DOM.find('#coachInput');
                if (input) {
                    input.value = 'Ich möchte eine Berndeutsch Zusammenfassung erstellen und ins Hochdeutsche übersetzen.';
                    askCoachKI();
                }
            }, 300);
        });
        
        quickAccess.appendChild(berndeutschBtn);
    }
}

// Initialize Berndeutsch Features
document.addEventListener('DOMContentLoaded', function() {
    if (initBerndeutschSpeechRecognition()) {
        console.log('🏔️ Berndeutsch Speech Recognition verfügbar');
    } else {
        console.log('❌ Berndeutsch Speech Recognition nicht verfügbar');
    }
    
    // Berndeutsch Button nach Initialisierung hinzufügen
    setTimeout(addBerndeutschQuickButton, 4000);
});

console.log('🏔️ Berndeutsch → Hochdeutsch Translation Feature geladen');
console.log('💡 Nutze Coach-KI Tab für Berndeutsch Voice Summaries mit Auto-Übersetzung');